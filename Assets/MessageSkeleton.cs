...
public class MessageSkeleton : MonoBehaviour {
	...
	private void Update()
	{
		if (Anim != null)
		{
			...
			//Right_Hip.transform.localRotation = frame.WorldRotations[8];

			Right_Knee.transform.position = transform.position + frame.Positions[9];
			//Right_Knee.transform.localRotation = frame.WorldRotations[9];

			//Left_Hip.transform.position = transform.position + frame.Positions[11];
			//Left_Hip.transform.localRotation = frame.WorldRotations[11];

			Left_Knee.transform.position = transform.position + frame.Positions[12];
			//Left_Knee.transform.localRotation = frame.WorldRotations[12];
		}
	}
}
